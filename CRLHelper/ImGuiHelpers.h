...
inline bool Combo(const char* label, int* idx, std::vector<std::string>& values) {
    ...
    return Combo(label, idx, vector_getter, static_cast<void*>(&values), values.size());
}
...
inline bool ListBox(const char* label, int* idx, std::vector<std::string>& values) {
    ...
    return ListBox(label, idx, vector_getter, static_cast<void*>(&values), values.size());
}
...
// template<typename T>
// inline bool DragScalar(const char *label, T* value, void* v, float v_speed, const void* v_min = NULL, const void*
// v_max = NULL, const char* format = NULL, float power = 1.0f)
// {
